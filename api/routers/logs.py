...
async def delete_meal(
    ...
    if account_data:
        message = repo.delete(meal_id)
        return message
    return {"message": "no account logged in"}


@router.get("/api/{account}/exercises", response_model=list[LogExercise] | dict)
...
async def create_exercise(
    ...
    if account_data:
        exercise = get_exercises(info.log_exercise, info.gender, info.weight_kg,
                                  info.height_cm, info.age)
